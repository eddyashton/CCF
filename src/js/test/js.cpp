...
TEST_CASE("JSWrappedValue copy assignment frees old value")
{
  ...
  {
    // Wrap both via JSWrappedValue::take (takes ownership, no dup)
    auto wa = ccf::js::core::JSWrappedValue::take(ctx, obj_a);
    auto wb = ccf::js::core::JSWrappedValue::take(ctx, obj_b);
