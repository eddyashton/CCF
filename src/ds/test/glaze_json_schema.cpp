...
TEST_CASE("glaze demo")
{
  // to_json equivalent auto-generated
  const Bar default_bar = {};
  const std::string json = glz::write_json(default_bar).value_or("error");
  REQUIRE(json != "error");

  // from_json equivalent auto-generated, all permissive by default
  // Note there is no way to mark _some_ fields required when converting _from_
  // JSON
  // https://github.com/stephenberry/glaze/blob/main/docs/FAQ.md#what-happens-when-the-json-input-is-missing-objects
  // Related but not quite right features:
  //  - `required` array in `glz::meta<T>`, but this appears to only affect
  //  schema generation, not parsing behaviour
  //  - `error_on_missing_keys` in `glz::opts`, but this is all-or-nothing and
  //  determined by the read call
  std::string s = "{}";
  auto parsed_bar = glz::read_json<Bar>(s);
  REQUIRE(parsed_bar.has_value());
  REQUIRE(parsed_bar->a == default_bar.a);
  REQUIRE(parsed_bar->b == default_bar.b);
  REQUIRE(parsed_bar->c == default_bar.c);

  s = R"({"a": 42})";
  parsed_bar = glz::read_json<Bar>(s);
  REQUIRE(parsed_bar.has_value());
  REQUIRE(parsed_bar->a == 42);
  REQUIRE(parsed_bar->b == default_bar.b);
  REQUIRE(parsed_bar->c == default_bar.c);

  s = R"({"a": 42, "b": "Test", "c": 100})";
  parsed_bar = glz::read_json<Bar>(s);
  REQUIRE(parsed_bar.has_value());
  REQUIRE(parsed_bar->a == 42);
  REQUIRE(parsed_bar->b == "Test");
  REQUIRE(parsed_bar->c == 100);

  // Also, some effort to pretty-printing for parse errors (if you still have
  // the source buffer)
  s = R"({"a": "Test"})";
  parsed_bar = glz::read_json<Bar>(s);
  REQUIRE_FALSE(parsed_bar.has_value());
  const auto formatted_error_1 = glz::format_error(parsed_bar, s);
  const std::string expected_error_1 = R"(1:7: parse_number_failure
   {"a": "Test"}
         ^)";
  REQUIRE(formatted_error_1 == expected_error_1);

  s = R"({"b": 42})";
  parsed_bar = glz::read_json<Bar>(s);
  REQUIRE_FALSE(parsed_bar.has_value());
  const auto formatted_error_2 = glz::format_error(parsed_bar, s);
  const std::string expected_error_2 = R"(1:7: expected_quote
   {"b": 42}
         ^)";
  REQUIRE(formatted_error_2 == expected_error_2);

  // One discrepancy - unknown keys are (by default) an error
  s = R"({"other": "Bob"})";
  parsed_bar = glz::read_json<Bar>(s);
  REQUIRE_FALSE(parsed_bar.has_value());
  const auto formatted_error_3 = glz::format_error(parsed_bar, s);
  const std::string expected_error_3 = R"(1:3: unknown_key
   {"other": "Bob"}
     ^)";
  REQUIRE(formatted_error_3 == expected_error_3);

  // Telling it not to do this is doable, but a little different - may need our
  // own defines to simplify
  Bar b;
  auto error = glz::read<glz::opts{.error_on_unknown_keys = false}>(b, s);
  REQUIRE_FALSE(error);
}
...
template <>
struct glz::meta<Bar2>
{
  using T = Bar2;
  static constexpr auto value = object(&T::a, &T::b, &T::c);
};

TEST_CASE("basic macro parser generation")
{
  std::string s = "{}";
  auto parsed_bar = glz::read_json<Bar2>(s);
  REQUIRE(parsed_bar.has_value());
}
...
TEST_CASE("mappings")
{
  ...
  // Surprising! Enums are stringed in JSON, but produce pair-arrays rather
than
...
{
  const auto schema =
ccf::ds::json::build_schema<renamed::Foo>("renamed::Foo");
